...
class SalesForecasting:
    ...
    def __fit_lstm_model(self, model):
        ...
        # 时间步长为1
        timesteps = X_train_lstm.shape[1]  # 时间步长
        features = X_train_lstm.shape[2]  # 特征数
        model.add(Input(batch_shape=(1, timesteps, features)))
        model.add(LSTM(4, stateful=True))
        #model.add(LSTM(4, batch_input_shape=(1, X_train_lstm.shape[1], X_train_lstm.shape[2]), 
        #           stateful=True))
        #model.add(LSTM(4, input_shape=(X_train_lstm.shape[1], X_train_lstm.shape[2]), stateful=True))
        model.add(Dense(1))
        model.add(Dense(1))
